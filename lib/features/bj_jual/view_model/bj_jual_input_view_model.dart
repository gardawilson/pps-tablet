...
// -----------------------------------------------------------------------------
// Small value objects (SAMA DENGAN PACKING VM)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// ViewModel (FORMAT SAMA DENGAN PACKING VM)
// -----------------------------------------------------------------------------
class BJJualInputViewModel extends ChangeNotifier {
  ...
  // ---------------------------------------------------------------------------
  // Debug control (SAMA DENGAN PACKING VM)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Inputs per row (cache, loading & error per NoBJJual) - SAMA DENGAN PACKING
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Master Cabinet Materials (optional) - SAMA DENGAN PACKING
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Lookup label (scan) - SAMA DENGAN PACKING
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Temporary data by label - REQUIRED by dialogs (SAMA DENGAN PACKING)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Temp selections (anti-duplicate) - SAMA DENGAN PACKING
  // ---------------------------------------------------------------------------
  ...
  // ⚠️ IMPORTANT: _tempKeys must track ctx.simpleKey(row), not your custom keys
  ...
  void _updateTempItemsByLabel(String labelCode) {
    ...
    final bjFull = tempBarangJadi.where((e) => _getItemLabelCode(e) == code).toList();
    final bjPart = tempBarangJadiPartial.where((e) => _getItemLabelCode(e) == code).toList();

    final fwFull = tempFurnitureWip.where((e) => _getItemLabelCode(e) == code).toList();
    final fwPart = tempFurnitureWipPartial.where((e) => _getItemLabelCode(e) == code).toList();
  ...
  // ---------------------------------------------------------------------------
  // Duplicate helpers (REQUIRED by dialogs) - SAMA DENGAN PACKING
  // ---------------------------------------------------------------------------
  Set<String> _dbKeysFor(String noBJJual) {
    final keys = <String>{};
    final db = _inputsCache[noBJJual];

    if (db == null) return keys;

    // IMPORTANT: should match ctx.simpleKey(row)
    // safest approach: store by label code of items (same as packing approach for FWIP)
    for (final it in db.barangJadi) {
      final code = _getItemLabelCode(it);
      if (code != null) keys.add(code.trim());
    }
    for (final it in db.furnitureWip) {
      final code = _getItemLabelCode(it);
      if (code != null) keys.add(code.trim());
    }

    // cabinet material usually not tracked by key (qty update), same as packing
    return keys;
  }

  Set<String> _allKeysFor(String noBJJual) {
    final all = _dbKeysFor(noBJJual);
  ...
  bool willBeDuplicate(Map<String, dynamic> row, String noBJJual) {
    ...
    return _allKeysFor(noBJJual).contains(sk.trim());
  }
  ...
  int countNewRowsInLastLookup(String noBJJual) {
    ...
    for (final row in ctx.data) {
      ...
      if (willBeDuplicate(row, noBJJual)) continue;
      fresh++;
    }
    return fresh;
  }
  ...
  void pickAllNew(String noBJJual) {
    ...
    for (int i = 0; i < ctx.data.length; i++) {
      ...
      if (_allKeysFor(noBJJual).contains(sk.trim())) continue;
      if (!ctx.isRowValid(row)) continue;
  ...
  // ---------------------------------------------------------------------------
  // Commit picked → TEMP (SAMA DENGAN PACKING STYLE)
  // ---------------------------------------------------------------------------
  ...
  TempCommitResult commitPickedToTemp({required String noBJJual}) {
    ...
    final Set<String> allKeys = _allKeysFor(noBJJual);
    ...
    for (int i = 0; i < typedItems.length; i++) {
      ...
      // skip if already present (db or temp)
      if (allKeys.contains(simpleKey)) {
        skipped++;
        continue;
      }

      // only accept BJ / FWIP from lookup
      bool itemAdded = false;

      if (item is BarangJadiItem) {
        if (isPartial || item.isPartialRow) {
          tempBarangJadiPartial.add(item);
        } else {
          tempBarangJadi.add(item);
        }
        itemAdded = true;
      } else if (item is FurnitureWipItem) {
        if (isPartial || item.isPartialRow) {
          tempFurnitureWipPartial.add(item);
        } else {
          tempFurnitureWip.add(item);
        }
        itemAdded = true;
      } else {
        skipped++;
  ...
  // ---------------------------------------------------------------------------
  // Cabinet Material TEMP (manual from master dropdown) - SAMA DENGAN PACKING
  // ---------------------------------------------------------------------------
  void addTempCabinetMaterialFromMaster({
    ...
    // material not tracked by ctx.simpleKey; keep debug key only
  ...
  // ---------------------------------------------------------------------------
  // Delete temp items - SAMA DENGAN PACKING
  // ---------------------------------------------------------------------------
  bool deleteIfTemp(dynamic item) {
    ...
    if (ok) {
      ...
      // IMPORTANT: also remove from _tempKeys if we can infer label code equals simpleKey
      final label = _getItemLabelCode(item);
      if (label != null) _tempKeys.remove(label.trim());
  ...
  // ---------------------------------------------------------------------------
// Delete ALL TEMP items for one labelCode (same pattern as packing)
// ---------------------------------------------------------------------------
  int deleteAllTempForLabel(String labelCode) {
    ...
    // copy first because deleteIfTemp mutates lists
  ...
  // ---------------------------------------------------------------------------
  // Submit temp items - SAMA DENGAN PACKING
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Delete inputs & partials (DB items) - keep like your current version
  // ---------------------------------------------------------------------------
  ...
  Map<String, dynamic> _buildDeletePayloadFromItems(List<dynamic> items) {
    ...
    for (final it in items) {
      if (it is BarangJadiItem) {
        final isPart = it.isPartialRow && (it.noBJPartial ?? '').trim().isNotEmpty;
        if (isPart) {
