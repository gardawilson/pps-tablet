...
/// Cek apakah bucket TEMP untuk labelCode berisi item partial (BJ / FWIP).
bool _hasPartialTempForLabel(
  ...
  for (final it in b.allItems) {
    if (it is BarangJadiItem) {
      if ((it.noBJPartial ?? '').trim().isNotEmpty) return true;
      if (it.isPartialRow == true) return true;
    }
    if (it is FurnitureWipItem) {
      if ((it.noFurnitureWIPPartial ?? '').trim().isNotEmpty) return true;
      if (it.isPartialRow == true) return true;
    }
  }
...
class InputsGroupTooltip extends StatefulWidget {
  ...
  /// ✅ Summary total builder (opsional)
          ...
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                // BODY
                Flexible(
                  child: Consumer<BJJualInputViewModel>(
...
/// Table row dengan itemData (SAMA)
...
class _GroupTooltipAnchorTileState extends State<GroupTooltipAnchorTile> {
  ...
  void _show() {
    ...
    final delAllLabel =
    hasPartialTemp ? 'Hapus Semua (TEMP Partial)' : 'Hapus Semua (TEMP)';
  ...
  Future<void> _handleDeleteAllTemp(BJJualInputViewModel vm) async {
