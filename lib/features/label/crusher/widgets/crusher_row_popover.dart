...
class CrusherRowPopover extends StatelessWidget {
  ...
  Future<void> _copyOnly(BuildContext context) async {
    await Clipboard.setData(ClipboardData(text: header.noCrusher));
    final m = ScaffoldMessenger.maybeOf(context);
    m?.hideCurrentSnackBar();
    m?.showSnackBar(
      SnackBar(
        content: Text('NoWashing "${header.noCrusher}" disalin'),
            ...
            _MenuTile(
              ...
              onTap: () => _runAndClose(() async {
                ...
                await pdfService.printReport80mm(
                  ...
                  query: {'NoCrusher': header.noCrusher},
                  // system: 'pps', // opsional kalau mau override
                  // saveNameHint: 'Label_${header.noCrusher}.pdf', // opsional; kalau kosong akan di-infer
                );
