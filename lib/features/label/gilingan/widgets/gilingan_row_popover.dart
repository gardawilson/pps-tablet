            ...
            children: [
              ...
              LabelPopoverMenuTile(
                ...
                onTap: () => _runAndClose(() async {
                  ...
                  final noGilingan = widget.header.noGilingan;
                  ...
                  try {
                    await lockApi.acquire(noGilingan);
                    isLockAcquired = true;

                    await PdfPrintService(defaultSystem: 'pps').previewFromUrl(
                      context: rootCtx,
                      pdfUrl: Uri.parse(
                        ApiConstants.gilinganLabelPdf(noGilingan),
                      ),
                      title: noGilingan,
                      onPrinted: () {
                        isPrinted = true;
                        () async {
                          ...
                          try {
                            final count = await repo.markAsPrinted(noGilingan);
                            if (count != null) {
                              lockVm.setPrintCount(noGilingan, count);
                            }
                          } catch (_) {
                            needsIncrement = true;
                          }
                          ...
                          try {
                            await lockApi.release(noGilingan);
                          } catch (_) {
                            needsRelease = true;
                          }
