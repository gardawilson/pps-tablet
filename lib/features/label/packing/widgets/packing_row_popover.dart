...
class PackingRowPopover extends StatefulWidget {
  ...
  @override
  State<PackingRowPopover> createState() => _PackingRowPopoverState();
}

class _PackingRowPopoverState extends State<PackingRowPopover> {
  ...
  Future<void> _copyOnly() async {
    await Clipboard.setData(ClipboardData(text: widget.header.noBJ));
    if (!mounted) return;
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            (widget.header.namaBJ ?? '').trim().isEmpty
                                ? 'Packing'
                                : widget.header.namaBJ!.trim(),
                            style: const TextStyle(
                              fontSize: 12,
                    ...
                    await PdfPrintService(defaultSystem: 'pps').previewFromUrl(
                      ...
                      onPrinted: () {
                        ...
                        () async {
                          ...
                          try {
                            final count = await repo.markAsPrinted(noBJ);
                            if (count != null) {
