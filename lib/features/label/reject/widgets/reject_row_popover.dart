...
class RejectRowPopover extends StatelessWidget {
  ...
  Future<void> _copyOnly(BuildContext context) async {
    await Clipboard.setData(ClipboardData(text: header.noReject));
    final m = ScaffoldMessenger.maybeOf(context);
    m?.hideCurrentSnackBar();
    m?.showSnackBar(
      SnackBar(
        content: Text('NoReject "${header.noReject}" disalin'),
  ...
  Widget build(BuildContext context) {
    ...
    final jenisText = 'ID Reject: ${header.idReject}';
          ...
          children: [
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: const Icon(
                      Icons.delete_outline, // icon lebih “reject”
                      color: Colors.white,
                      size: 20,
                    ),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        // ID Reject + proses (kode + mesin/BJ Sortir)
                        Text(
                          '$jenisText • $prosesText',
                          style: TextStyle(
                            fontSize: 12,
            ...
            _MenuTile(
              ...
              onTap: () => _runAndClose(() async {
                ...
                // misal: 'CrLabelReject' atau nama lain di laporan Reject
                await pdfService.printReport80mm(
                  context: rootCtx,
                  reportName: 'CrLabelRejectV2',
                  query: {'NoReject': header.noReject},
                  // saveNameHint: 'Label_${header.noReject}.pdf',
                );
              }),
            ),
            divider,
...
/// Tile menu – dipakai ulang (sama dengan Packing)
