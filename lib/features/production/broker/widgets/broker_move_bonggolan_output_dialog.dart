...
class BrokerMoveBonggolanOutputDialog extends StatefulWidget {
  final String noProduksi;
  final List<BonggolanOutput> outputs;

  const BrokerMoveBonggolanOutputDialog({
    super.key,
  ...
  @override
  State<BrokerMoveBonggolanOutputDialog> createState() =>
      _BrokerMoveBonggolanOutputDialogState();
}

class _BrokerMoveBonggolanOutputDialogState
    extends State<BrokerMoveBonggolanOutputDialog> {
  final _searchCtrl = TextEditingController();
  final _scrollCtrl = ScrollController();
  final _repo = BrokerProductionRepository();

  final Set<String> _selectedNoBonggolan = {};
  ...
  Future<void> _submit() async {
    if (_selectedTarget == null || _selectedNoBonggolan.isEmpty || _isSubmitting) return;

    setState(() => _isSubmitting = true);
    final vm = context.read<BrokerProductionInputViewModel>();
    final success = await vm.moveBonggolanOutputs(
      widget.noProduksi,
      _selectedTarget!.noProduksi,
      _selectedNoBonggolan.toList(),
    );
    ...
    if (success) {
      ...
      final errMsg = vm.moveBonggolanOutputError ?? 'Gagal memindahkan output bonggolan';
      await showDialog(
        context: context,
        builder: (_) =>
          ...
          children: [
            ...
            Expanded(
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  Expanded(child: _buildBonggolanSelector()),
  ...
  // ---------------------------------------------------------------------------
  // Left panel — bonggolan selector
  // ---------------------------------------------------------------------------

  Widget _buildBonggolanSelector() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Container(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              if (_selectedNoBonggolan.isNotEmpty)
                Container(
                  padding:
                  ...
                  child: Text(
                    '${_selectedNoBonggolan.length} dipilih',
          ...
          child: widget.outputs.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final noBonggolan = output.noBonggolan ?? '';
                    final isSelected =
                        _selectedNoBonggolan.contains(noBonggolan);
                    return _BonggolanOutputItem(
                      output: output,
                      ...
                      onTap: () {
                        setState(() {
                          if (isSelected) {
                            _selectedNoBonggolan.remove(noBonggolan);
                          } else {
                            _selectedNoBonggolan.add(noBonggolan);
                          }
                        });
                      },
                    );
  ...
  Widget _buildFooter() {
    final canSubmit = _selectedTarget != null &&
        _selectedNoBonggolan.isNotEmpty &&
        !_isSubmitting;
...
// ---------------------------------------------------------------------------
// Bonggolan output item (left panel)
// ---------------------------------------------------------------------------

class _BonggolanOutputItem extends StatelessWidget {
  final BonggolanOutput output;
  final bool isSelected;
  final bool disabled;
  final VoidCallback onTap;

  const _BonggolanOutputItem({
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  if ((output.namaBonggolan ?? '').isNotEmpty)
                    Text(
