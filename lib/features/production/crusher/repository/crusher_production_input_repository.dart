...
// -----------------------------
  ...
  // GET: Formula Inputs
  // -----------------------------
  /// GET /api/production/crusher/:noCrusherProduksi/formula-inputs
  Future<({Set<String> kodes, List<ProductionFormulaOutput> outputs})>
      fetchFormulaInputs(String noCrusherProduksi) async {
    final key = noCrusherProduksi.trim();
    if (key.isEmpty) throw ArgumentError('noCrusherProduksi tidak boleh kosong');

    final token = await TokenStorage.getToken();
    final url = Uri.parse('$_base/api/production/crusher/$key/formula-inputs');
