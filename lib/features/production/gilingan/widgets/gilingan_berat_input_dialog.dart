// lib/features/production/gilingan/widgets/gilingan_berat_input_dialog.dart
// Weight input dialog for Bonggolan (M.), Crusher (F.), and Reject (BF.) items.
...
const _kGilinganBeratColor = Color(0xFF0277BD);

class GilinganBeratInputDialog extends StatefulWidget {
  final String noProduksi;

  const GilinganBeratInputDialog({super.key, required this.noProduksi});

  @override
  State<GilinganBeratInputDialog> createState() =>
      _GilinganBeratInputDialogState();
}

class _GilinganBeratInputDialogState extends State<GilinganBeratInputDialog> {
  final TextEditingController _ctrl = TextEditingController();
  bool _ready = false;
  double? _originalBerat;
  String _labelCode = '-';
  String _namaJenis = '-';
  String _categoryLabel = 'INPUT';
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (result != null && result.typedItems.isNotEmpty) {
        ...
        if (item is BonggolanItem) {
          _labelCode = item.noBonggolan ?? '-';
          _namaJenis = item.namaJenis ?? '-';
          _originalBerat = item.berat;
          _categoryLabel = 'BONGGOLAN';
        } else if (item is CrusherItem) {
          _labelCode = item.noCrusher ?? '-';
          _namaJenis = item.namaJenis ?? '-';
          _originalBerat = item.berat;
          _categoryLabel = 'CRUSHER';
        } else if (item is RejectItem) {
          _labelCode = item.noReject ?? '-';
          _namaJenis = item.namaJenis ?? '-';
          _originalBerat = item.berat;
          _categoryLabel = 'REJECT';
        }
  ...
  void _commit(GilinganProductionInputViewModel vm) {
    final result = vm.lastLookup;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          r.added > 0
              ? '✅ $_categoryLabel ditambahkan${isPartial ? ' (partial)' : ''}'
                              ...
                              child: InkWell(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    borderRadius: BorderRadius.circular(10),
                                    border: Border.all(
                                      color: _kGilinganBeratColor.withValues(
                                        alpha: 0.4,
                                      ),
                                    ),
                                  ),
                      ...
                      Expanded(
                        child: FilledButton(
                          ...
                          style: FilledButton.styleFrom(
                            backgroundColor: _kGilinganBeratColor,
