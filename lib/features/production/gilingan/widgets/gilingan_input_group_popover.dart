// =============================
// lib/features/production/gilingan/widgets/gilingan_input_group_popover.dart
// Copy dari broker popover, ganti VM -> Gilingan
// =============================
...
// ✅ Import model item yang mungkin muncul di bucket.allItems
// (sesuaikan path kalau kamu punya aggregator model khusus gilingan)
...
bool _hasPartialTempForLabel(
  ...
  // ✅ Aman: pakai allItems (tidak tergantung field b.bbItems/b.gilinganItems/...)
  for (final it in b.allItems) {
    if (it is BbItem) {
      if ((it.noBBPartial ?? '').trim().isNotEmpty) return true;
    } else if (it is GilinganItem) {
      if ((it.noGilinganPartial ?? '').trim().isNotEmpty) return true;
    } else if (it is MixerItem) {
      if ((it.noMixerPartial ?? '').trim().isNotEmpty) return true;
    } else if (it is RejectItem) {
      if ((it.noRejectPartial ?? '').trim().isNotEmpty) return true;
    }
  }
  ...
  Future<void> _confirmBulkDelete() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Yakin ingin menghapus $count item yang sudah dipilih?'),
        actions: [
          TextButton(
          ...
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                // TABLE HEADER
                if (widget.tableHeaders != null && widget.tableHeaders!.isNotEmpty) ...[
                  Container(
                    ...
                    child: Row(
                      children: [
                        if (_selectionMode && canDeleteExisting) const SizedBox(width: 36),
                          ...
                          return Expanded(
                            ...
                            child: Padding(
                              padding: EdgeInsets.only(
                                right: index < widget.tableHeaders!.length - 1 ? 8 : 0,
                              ),
                              child: Text(
                                header,
                                style: TextStyle(
                ...
                // BODY
                Flexible(
                  child: Consumer<GilinganProductionInputViewModel>(
                    builder: (context, vm, _) {
                      ...
                      return ListView.separated(
                        shrinkWrap: true,
                        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                        itemBuilder: (_, i) => _buildSelectableRow(children[i], i),
                ...
                if (widget.summaryBuilder != null)
                  Consumer<GilinganProductionInputViewModel>(
                    builder: (context, vm, _) {
                              ...
                              if (flexes.isEmpty) {
                                ...
                                for (int i = 0; i < flexes.length; i++) {
                                  if (i == flexes.length - 1) {
                                    ...
                                    cols.add(Expanded(flex: flexes[i], child: const SizedBox.shrink()));
                                  }
                                }
                              }
                ...
                Padding(
                  ...
                  child: Row(
                    children: _selectionMode && canDeleteExisting
                      ...
                      const Spacer(),
                      FilledButton.icon(
                        onPressed: _selectedIndices.isEmpty ? null : _confirmBulkDelete,
                      ...
                      if (widget.onDeleteAllTemp != null)
                        TextButton.icon(
                          onPressed: widget.deleteAllTempDisabled ? null : widget.onDeleteAllTemp,
...
class _GroupTooltipAnchorTileState extends State<GroupTooltipAnchorTile> {
  ...
  Future<void> _handleDeleteAllTemp(GilinganProductionInputViewModel vm) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          removed > 0 ? 'Berhasil menghapus $removed item TEMP.' : 'Tidak ada item TEMP untuk dihapus.',
        ),
        behavior: SnackBarBehavior.floating,
