// lib/features/shared/gilingan_production/widgets/hot_stamp_production_dropdown.dart
...
class GilinganProductionDropdown extends StatefulWidget {
  ...
  final ValueChanged<GilinganProduction?>? onChanged;
  ...
  const GilinganProductionDropdown({
    super.key,
    this.preselectNoProduksi,
    this.preselectNamaMesin,
    this.onChanged,
    this.label = 'Proses Gilingan',
  ...
  @override
  State<GilinganProductionDropdown> createState() =>
      _GilinganProductionDropdownState();
}

class _GilinganProductionDropdownState
    extends State<GilinganProductionDropdown> {
  GilinganProduction? _value;
  bool _usePreselectedOnly = false;
  List<GilinganProduction> _localItems = const [];
  ...
  @override
  void didUpdateWidget(covariant GilinganProductionDropdown oldWidget) {
    super.didUpdateWidget(oldWidget);

    // In edit mode we keep the synthetic single item and ignore date changes
  ...
  Future<void> _primeOrFetch() async {
    ...
    if (pre.isNotEmpty) {
      // EDIT MODE → build 1 synthetic local item
      final item = GilinganProduction(
        noProduksi: pre,
        idOperator: 0,
        namaOperator: '',
        idMesin: 0,
  ...
  Future<void> _fetchForCurrentDate() async {
    ...
    try {
      ...
      vm.error = 'Gagal memuat data produksi gilingan';
    } finally {
      vm.isLoading = false;
      vm.notifyListeners();
    }
  ...
  Widget build(BuildContext context) {
    return Consumer<GilinganProductionViewModel>(
      builder: (context, vm, _) {
        ...
        return DropdownPlainField<GilinganProduction>(
          label: widget.label,
