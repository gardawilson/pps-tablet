// lib/features/production/hot_stamping/view_model/hot_stamping_production_input_view_model.dart
...
// ViewModel
// -----------------------------------------------------------------------------
class HotStampingProductionInputViewModel extends ChangeNotifier {
  final HotStampingProductionInputRepository repository;
  HotStampingProductionInputViewModel({required this.repository});
  ...
  void _d(String message) {
    if (kDebugMode && _verbose) debugPrint('[HotStampInputVM] $message');
  }
  ...
  final Map<String, Future<HotStampingInputs>> _inflight = {};
  ...
  HotStampingInputs? inputsOf(String noProduksi) => _inputsCache[noProduksi];
  int inputsCount(String noProduksi, String key) =>
      _inputsCache[noProduksi]?.summary[key] ?? 0;

  Future<HotStampingInputs?> loadInputs(
    String noProduksi, {
    bool force = false,
  }) async {
  ...
  // ---------------------------------------------------------------------------
  // Lookup FWIP label (masih dipakai untuk scan FWIP)
  // ---------------------------------------------------------------------------
  ...
  Future<ProductionLabelLookupResult?> lookupFwipLabel(
    String code, {
    bool force = false,
  }) async {
  ...
  Set<String> _dbKeysFor(String noProduksi) {
    ...
    if (db != null) {
      ...
      // cabinet material: tidak ikut key-tracking di sini karena bisa di-update jumlahnya
    }
    return keys;
  }
  ...
  /// ✅ Delete FWIP item from temp (lokal only)
  ...
  /// ✅ Delete cabinet material from temp (lokal only)
  ...
  /// ✅ Delete all temp items for specific label (FWIP only)
  ...
  /// ✅ Build payload for submit (FWIP + Cabinet Material)
  ...
  /// ✅ Build delete payload from items (FWIP + Cabinet Material)
  ...
  /// - Items di DB akan dihapus via API
