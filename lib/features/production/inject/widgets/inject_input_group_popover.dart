// lib/features/production/inject_production/widgets/inject_input_group_popover.dart
...
bool _hasPartialTempForLabel(
  ...
  final hasBrokerPart =
      b.brokerPartials.isNotEmpty ||
          b.brokerItems.any((e) => (e.noBrokerPartial ?? '').trim().isNotEmpty);

  final hasMixerPart =
      b.mixerPartials.isNotEmpty ||
          b.mixerItems.any((e) => (e.noMixerPartial ?? '').trim().isNotEmpty);

  final hasGilinganPart =
      b.gilinganPartials.isNotEmpty ||
          b.gilinganItems.any((e) => (e.noGilinganPartial ?? '').trim().isNotEmpty);

  final hasFwipPart =
      b.furnitureWipPartials.isNotEmpty ||
          b.furnitureWipItems.any((e) => (e.noFurnitureWIPPartial ?? '').trim().isNotEmpty);

  return hasBrokerPart || hasMixerPart || hasGilinganPart || hasFwipPart;
}
          ...
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Builder(
                          builder: (_) {
                            final vm =
                            context.read<InjectProductionInputViewModel>();
                ...
                // =======================================================
                Flexible(
                  child: Consumer<InjectProductionInputViewModel>(
                ...
                // =======================================================
                // ✅ SUMMARY SECTION (Total Berat + Pcs Sejajar)
                // =======================================================
                if (widget.summaryBuilder != null)
                  Consumer<InjectProductionInputViewModel>(
                    builder: (context, vm, _) {
                          ...
                          children: [
                            ...
                            ...() {
                              ...
                              if (flexes.isEmpty) {
                                ...
                                // Show berat atau pcs tergantung mana yang ada
                                if (summary.totalBerat > 0) {
                                  summaryColumns.add(
                                    Expanded(
                                      child: Padding(
                                        padding: const EdgeInsets.only(right: 8),
                                ...
                                for (int i = 0; i < flexes.length; i++) {
                                  if (i == flexes.length - 1) {
                                    // Last column: show berat atau pcs
                                    String valueText = '-';
                                    if (summary.totalBerat > 0) {
                                      valueText = '${summary.totalBerat.toStringAsFixed(2)} kg';
                                    } else if (summary.totalPcs > 0) {
                                      valueText = '${summary.totalPcs} pcs';
                                    }
...
class _GroupTooltipAnchorTileState extends State<GroupTooltipAnchorTile> {
  ...
  Future<void> _handleDeleteAllTemp(
      InjectProductionInputViewModel vm) async {
