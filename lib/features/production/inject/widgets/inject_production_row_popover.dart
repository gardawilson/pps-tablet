// lib/features/production/inject/widgets/inject_production_row_popover.dart
...
class InjectProductionRowPopover extends StatelessWidget {
  final InjectProduction row;
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(
                        Icons.precision_manufacturing, // ✅ Inject icon
                        color: Colors.white,
                        size: 20,
                      ),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          // ✅ Additional info: Jam atau Jam Range
  ...
  /// ✅ Helper untuk build jam info (mirip dengan Mixer yang show "X jam kerja")
  String _buildJamInfo() {
    // Prioritas: tampilkan jam range kalau ada
    final s = (row.hourStart ?? '').trim();
    final e = (row.hourEnd ?? '').trim();

    if (s.isNotEmpty && e.isNotEmpty) {
      return '$s - $e';
    }

    // Fallback: tampilkan jam saja
    if (row.jam != null && row.jam! > 0) {
      return '${row.jam} jam kerja';
    }
