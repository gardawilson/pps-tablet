...
class _MixerProductionFormDialogState
  ...
  Future<void> _submit() async {
    ...
    if (!isEdit && _selectedMixerType == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Jenis mixer wajib dipilih')),
      );
      return;
    }
    ...
    try {
      if (isEdit) {
        ...
        result = await prodVm.createProduksiWithJenis(
          ...
          outputJenisId: _selectedMixerType!.idMixer,
          shift: _selectedShift!,
          hourStart: hourStartSql,
          hourEnd: hourEndSql,
      ...
      child: Column(
        ...
        children: [
          ...
          MixerTypeDropdown(
            ...
            onChanged: (mt) => setState(() => _selectedMixerType = mt),
            validator: isEdit
                ? null
                : (v) => v == null ? 'Wajib pilih jenis mixer' : null,
            autovalidateMode: AutovalidateMode.onUserInteraction,
          ),
