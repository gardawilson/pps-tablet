// lib/features/production/packing/view/packing_production_input_screen.dart
...
import '../widgets/packing_input_group_popover.dart';
...
class _PackingProductionInputScreenState extends State<PackingProductionInputScreen> {
  ...
  Future<void> _onCodeReady(BuildContext context, String code) async {
    ...
    // ✅ packing: pakai lookupLabel / lookupFwipLabel sesuai vm kamu
  ...
  Future<void> _handleSelectMode(
    ...
    await showDialog(
      ...
      builder: (_) => PackingLookupLabelDialog(
        noPacking: widget.noProduksi, // ✅ widget dialog tadi pakai noPacking
        selectedMode: _selectedMode,
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<PackingProductionInputViewModel>(
      builder: (context, vm, _) {
        ...
        // ✅ GANTI permission key packing kamu
        // contoh: final canDeleteByPerm = perm.can('label_packing:delete');
        ...
        return WillPopScope(
          ...
          child: Scaffold(
            appBar: AppBar(
              title: Text('Packing Inputs • ${widget.noProduksi}'),
              leading: IconButton(
                                      ...
                                      detailsBuilder: () {
                                        ...
                                        final dbItems = currentInputs == null
                                            ...
                                            : currentInputs.furnitureWip.where(
                                                (x) => fwipTitleKey(x) == entry.key);

                                        final tempFull = vm.tempFurnitureWip.where(
                                                (x) => fwipTitleKey(x) == entry.key);
                                        final tempPart = vm.tempFurnitureWipPartial.where(
                                                (x) => fwipTitleKey(x) == entry.key);
                                ...
                                builder: (context) {
                                  ...
                                  return CabinetMaterialCard(
                                    ...
                                    onAdd: locked
                                        ...
                                        : () {
                                      final vm =
                                      context.read<PackingProductionInputViewModel>();

                                      showDialog(
                                        context: context,
                                        builder: (_) => AddCabinetMaterialDialog(
                                          idWarehouse: 5, // ✅ ganti warehouse packing
                                          loadMaterials: ({
                                    ...
                                    onDeleteExisting: (item) async {
                                      ...
                                      final success =
                                      await vm.deleteItems(widget.noProduksi, [item]);
