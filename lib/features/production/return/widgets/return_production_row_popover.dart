// lib/features/shared/return_production/widgets/return_production_row_popover.dart
...
class ReturnProductionRowPopover extends StatelessWidget {
  final ReturnProduction row;
  ...
  Future<void> _copyOnly(BuildContext context) async {
    await Clipboard.setData(ClipboardData(text: row.noRetur));
    final m = ScaffoldMessenger.maybeOf(context);
    m?.hideCurrentSnackBar();
    m?.showSnackBar(
      SnackBar(
        content: Text('NoRetur "${row.noRetur}" disalin'),
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ replace with your real permission keys
    ...
    if (row.namaPembeli.trim().isNotEmpty) {
      subtitleParts.add(row.namaPembeli.trim());
    }
    if (row.invoice.trim().isNotEmpty) {
      subtitleParts.add('INV ${row.invoice.trim()}');
    }
    final subtitle = subtitleParts.isEmpty ? '-' : subtitleParts.join(' • ');

    final trailingInfo = (row.noBJSortir.trim().isNotEmpty)
        ? 'Sortir: ${row.noBJSortir.trim()}'
        : '';
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Icon(
                        row.isLocked ? Icons.lock : Icons.assignment_return,
                        color: Colors.white,
                        size: 20,
                      ),
                    ),
