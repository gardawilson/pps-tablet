...
class StokBahanBakuPakaiRepository {
  static const _timeout = Duration(seconds: 25);

  Future<List<StokBahanBakuItem>> fetchStok() async {
    final token = await TokenStorage.getToken();
    final url = Uri.parse(ApiConstants.stokBahanBakuPakai);
  ...
  Future<List<BahanBakuProsesLabel>> fetchLabel(int idBB) async {
    final token = await TokenStorage.getToken();
    final url = Uri.parse(ApiConstants.stokBahanBakuPakaiLabel(idBB));
