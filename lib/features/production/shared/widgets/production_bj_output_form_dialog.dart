...
import '../../../label/packing/view_model/packing_view_model.dart';
...
class ProductionBjOutputFormDialog extends StatelessWidget {
  const ProductionBjOutputFormDialog({
    super.key,
    required this.noProduksi,
    this.tglProduksi,
    this.accentColor = const Color(0xFF00796B),
    this.mode = PackingInputMode.inject,
    this.lockedIdJenis,
    ...
    return MultiProvider(
      providers: [
        ChangeNotifierProvider(
          create: (_) => PackingViewModel(
              repository: PackingRepository(api: ApiClient())),
        ),
        if (lockedIdJenis == null)
          ChangeNotifierProvider(
            create: (_) => MstBarangJadiViewModel(),
          ),
      ],
      child: _BjOutputFormBody(
...
class _BjOutputFormBody extends StatefulWidget {
  const _BjOutputFormBody({
    required this.noProduksi,
  ...
  @override
  State<_BjOutputFormBody> createState() => _BjOutputFormBodyState();
}

class _BjOutputFormBodyState extends State<_BjOutputFormBody> {
  final _pcsCtrl = TextEditingController();

  MstBarangJadi? _selectedJenis;
  String? _jenisErr;
  String? _pcsErr;
  String? _saveError;
  ...
  Future<void> _save() async {
    ...
    final idJenis = _isLocked ? widget.lockedIdJenis! : _selectedJenis?.idJenis;

    setState(() {
      _jenisErr = idJenis == null ? 'Pilih jenis barang jadi' : null;
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'PCS',
                      prefixIcon:
                          const Icon(Icons.filter_1_outlined, size: 16),
                      isDense: true,
