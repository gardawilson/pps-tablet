...
class SortirRejectProductionRowPopover extends StatelessWidget {
  final SortirRejectProduction row;
  ...
  Future<void> _copyOnly(BuildContext context) async {
    await Clipboard.setData(ClipboardData(text: row.noBJSortir));
    final m = ScaffoldMessenger.maybeOf(context);
    m?.hideCurrentSnackBar();
    m?.showSnackBar(
      SnackBar(
        content: Text('NoBJSortir "${row.noBJSortir}" disalin'),
  ...
  Widget build(BuildContext context) {
    ...
    // contoh: "sortir_reject:update" / "sortir_reject:delete"
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Icon(
                        row.isLocked ? Icons.lock : Icons.rule_folder_outlined,
                        color: Colors.white,
                        size: 20,
                      ),
                    ),
