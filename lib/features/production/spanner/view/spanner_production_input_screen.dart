...
class SpannerProductionInputScreen extends StatefulWidget {
  ...
  @override
  State<SpannerProductionInputScreen> createState() =>
      _SpannerProductionInputScreenState();
}
...
class _SpannerProductionInputScreenState
  ...
  Future<void> _openSplitDialog() async {
    ...
    >(
      ...
      showSplitDialog: (idMesin, tanggal) =>
          showDialog<({SpannerProduction prod, String namaJenis})>(
            context: context,
            barrierDismissible: false,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(height: 1, color: _kSpannerBorder),
          Expanded(
            child: Padding(
                              ...
                              if (_selectedInputTab == 'fwip') ...[
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      _SpannerInputSummaryBar(
                                        totalLabel: fwipLabelCount,
                                        totalPcs: fwipPcs,
                                        color: _kSpannerPrimary,
                                      ),
                                      const SizedBox(height: 10),
                                      _SpannerGrandTotalBar(
                                ...
                                FloatingActionButton(
                                  heroTag: 'fab_scan_spanner_input',
                                  mini: true,
  ...
  Widget _buildOutputPanel({
    required List<SpannerOutput> outputs,
    ...
    return Container(
      decoration: productionPanelDecoration(
        borderColor: _kSpannerOutput.withValues(alpha: 0.3),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
                                  ...
                                  children: [
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          if (isRejectTab)
                                            _SpannerRejectSummaryBar(
                                              totalLabel:
                                                  selectedOutputs.length,
                                              totalBerat: totalRejectBerat,
                                            )
                                          else
                                            _SpannerOutputSummaryBar(
                                              totalLabel:
                                                  selectedOutputs.length,
                                              totalPcs: totalPcs,
                                            ),
                                    ...
                                    FloatingActionButton(
                                      heroTag: isRejectTab
                                          ? 'fab_add_spanner_reject_output'
                                          : 'fab_add_spanner_output',
                                      mini: true,
                                      backgroundColor: locked
                                          ? Colors.grey.shade300
                                          : _kSpannerOutput,
  ...
  Widget build(BuildContext context) {
    return Consumer<SpannerProductionInputViewModel>(
      builder: (context, vm, _) {
        ...
        return PopScope(
          ...
          child: Scaffold(
            backgroundColor: _kSpannerSurface,
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _kSpannerBorder),
      ),
      child: InkWell(
        borderRadius: BorderRadius.circular(10),
...
class _SpannerOutputDetailDialog extends StatelessWidget {
  final SpannerOutput output;

  const _SpannerOutputDetailDialog({required this.output});
...
class _SpannerInputSummaryBar extends StatelessWidget {
  const _SpannerInputSummaryBar({
...
class _SpannerGrandTotalBar extends StatelessWidget {
  const _SpannerGrandTotalBar({
...
class _SpannerMaterialSummaryBar extends StatelessWidget {
  const _SpannerMaterialSummaryBar({required this.items, required this.color});
...
class _SpannerOutputSummaryBar extends StatelessWidget {
  const _SpannerOutputSummaryBar({
    required this.totalLabel,
    required this.totalPcs,
  });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _kSpannerOutput.withValues(alpha: 0.06),
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: _kSpannerOutput.withValues(alpha: 0.18)),
      ),
      child: Row(
        children: [
          ProductionInlineStat(
...
class _SpannerRejectSummaryBar extends StatelessWidget {
  const _SpannerRejectSummaryBar({
    required this.totalLabel,
...
class _SpannerOutputOverallBar extends StatelessWidget {
  const _SpannerOutputOverallBar({
    required this.totalLabel,
