...
import '../repository/spanner_production_input_repository.dart';
...
// ViewModel
// -----------------------------------------------------------------------------
class SpannerProductionInputViewModel extends ChangeNotifier {
  final SpannerProductionInputRepository repository;
  SpannerProductionInputViewModel({required this.repository});
  ...
  void _d(String message) {
    if (kDebugMode && _verbose) debugPrint('[SpannerInputVM] $message');
  }
  ...
  final Map<String, Future<SpannerInputs>> _inflight = {};
  ...
  SpannerInputs? inputsOf(String noProduksi) => _inputsCache[noProduksi];
  int inputsCount(String noProduksi, String key) =>
      _inputsCache[noProduksi]?.summary[key] ?? 0;

  Future<SpannerInputs?> loadInputs(
    String noProduksi, {
    bool force = false,
  }) async {
  ...
  // Outputs (FWIP + Reject)
  // ---------------------------------------------------------------------------
  final Map<String, List<SpannerOutput>> _outputsCache = {};
  final Map<String, bool> _outputsLoading = {};
  final Map<String, String?> _outputsError = {};

  List<SpannerOutput>? outputsOf(String noProduksi) =>
