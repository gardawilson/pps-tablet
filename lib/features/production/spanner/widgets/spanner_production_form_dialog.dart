...
class _SpannerProductionFormDialogState
  ...
  static const int _idBagianSpanner = 9;
    ...
    if (h != null) {
      if (h.idRegu != null) {
        _selectedRegu = MstRegu(
          idRegu: h.idRegu!,
          idBagian: _idBagianSpanner,
          namaRegu: h.namaRegu ?? '',
        );
      }
      if (h.idOperators.isNotEmpty) {
        final names =
            h.namaOperator.split(',').map((s) => s.trim()).toList();
  ...
  Future<void> _submit() async {
    ...
    if (_selectedReguId == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Regu wajib dipilih')));
      return;
    }
      ...
      child: Column(
        ...
        children: [
          ...
          FurnitureWipTypeDropdown(
            ...
            label: 'Jenis Output',
            hintText: 'Pilih jenis output spanner',
