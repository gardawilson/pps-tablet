// lib/features/production/washing/widgets/washing_production_output_form_dialog.dart
//
// Dialog tambah output washing — format identik dengan BrokerProductionOutputFormDialog.
// POST /api/production/washing/:noProduksi/outputs
...
const _kOutput = Color(0xFF00796B); // teal output washing
const _kBorder = Color(0xFFE2E6EA);

class WashingProductionOutputFormDialog extends StatefulWidget {
  ...
  const WashingProductionOutputFormDialog({
    super.key,
  ...
  @override
  State<WashingProductionOutputFormDialog> createState() =>
      _WashingProductionOutputFormDialogState();
}

class _WashingProductionOutputFormDialogState
    extends State<WashingProductionOutputFormDialog> {
  // ── inline input state ────────────────────────────────────────────────────
  ...
  double get _totalBerat =>
      _details.fold(0.0, (s, e) => s + (e.berat ?? 0.0));
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        ...
        // idLayanan 3 = washing (sesuaikan jika berbeda)
        final def = await svc.get(3);
        if (!mounted) return;
        setState(() {
  ...
  Future<void> _save() async {
    if (_details.isEmpty) {
      setState(
        () => _saveError = 'Tambah minimal 1 sak terlebih dahulu.',
      );
      return;
    }
    ...
    try {
      final noWashing = await widget.repository.createOutput(
      ...
      if (!mounted) return;
      Navigator.of(context).pop(noWashing);
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _isSaving = false;
    ...
    return LayoutBuilder(
      builder: (_, c) => GridView.builder(
        ...
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount:
              c.maxWidth < 300 ? 3 : (c.maxWidth < 400 ? 4 : 5),
          crossAxisSpacing: 8,
          mainAxisSpacing: 8,
