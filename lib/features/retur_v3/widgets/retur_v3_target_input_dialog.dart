...
/// Hasil input dialog tambah target pengganti — dikirim balik ke pemanggil
/// lewat `Navigator.pop(result)`.
class ReturV3TargetInput {
  final String kodeKategori;
  final int idJenis;
  final String namaJenis;
  final int pcs;

  const ReturV3TargetInput({
...
/// Dialog tambah target pengganti untuk sebuah item retur: toggle kategori
/// (Barang Jadi / Furniture WIP), dropdown jenis sesuai kategori, input pcs.
/// Tidak ada toggle Bagus/Rusak — itu atribut item retur yang KEMBALI, bukan
/// atribut target pengganti yang akan DIKIRIM.
class ReturV3TargetInputDialog extends StatefulWidget {
  const ReturV3TargetInputDialog({super.key});

  @override
  State<ReturV3TargetInputDialog> createState() =>
      _ReturV3TargetInputDialogState();
}

class _ReturV3TargetInputDialogState extends State<ReturV3TargetInputDialog> {
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? _kPrimary.withValues(alpha: 0.1) : Colors.white,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
