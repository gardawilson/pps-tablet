// lib/features/verifikasi/view/verifikasi_pc_dialog.dart
...
/// Dialog verifikasi Production Controller (badge "PC"; penugasan &
/// kehadiran regu) untuk satu NoProduksi — berlaku lintas jenis produksi
/// (washing, broker, dst) selama [VerifikasiAdapter.hasOperatorVerification]
/// jenis tsb true. Modul verifikasi yang berdiri sendiri: bisa diisi kapan
/// pun tanpa menunggu verifikasi Stock Controller — keduanya tidak saling
/// bergantung urutan. Hanya verifikasi Kadept (department) yang butuh
/// keduanya sudah tuntas (lihat `verifikasi_kd_dialog.dart`).
Future<void> showVerifikasiPcDialog(
  ...
  return showDialog<void>(
    ...
    builder: (_) => ChangeNotifierProvider<VerifikasiViewModel>.value(
      value: vm,
      child: VerifikasiPcDialog(item: item),
    ),
  );
}

class VerifikasiPcDialog extends StatefulWidget {
  final VerifikasiItem item;

  const VerifikasiPcDialog({super.key, required this.item});

  @override
  State<VerifikasiPcDialog> createState() => _VerifikasiPcDialogState();
}
...
class _PcDialogData {
  ...
  const _PcDialogData({required this.header, required this.crossCheck});
}

class _VerifikasiPcDialogState extends State<VerifikasiPcDialog> {
  late Future<_PcDialogData> _future;
  ...
  Future<_PcDialogData> _load(VerifikasiViewModel vm) async {
    final results = await Future.wait([
      vm.fetchOperatorHeader(widget.item),
      vm.fetchCrossCheck(widget.item),
    ]);
    return _PcDialogData(
  ...
  Future<void> _handleVerify() async {
    final confirmed = await showVerifikasiConfirmDialog(
      context,
      title: 'Verifikasi Production Controller?',
      message:
          'Konfirmasi bahwa penugasan operator & kehadiran regu untuk '
          '${widget.item.noProduksi} sudah dicek dan sesuai. Tindakan ini tidak bisa dibatalkan.',
    );
    ...
    final vm = context.read<VerifikasiViewModel>();
    final ok = await vm.verifyOperatorStage(widget.item);
    if (!mounted) return;
    ...
    if (ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('✅ Production Controller berhasil diverifikasi'),
  ...
  Widget build(BuildContext context) {
    return Dialog(
      ...
      child: ConstrainedBox(
        ...
        child: FutureBuilder<_PcDialogData>(
          future: _future,
                ...
                Flexible(
                  child: SingleChildScrollView(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        _buildOperatorPanel(data.header),
                        const Divider(height: 1, color: kVerifikasiBorder),
                        ReconciliationSection(
                          summary: data.crossCheck,
                          showLabels: false,
                        ),
                      ],
                    ),
  ...
  // ── Header: grid label-value dense, sama gaya dengan dialog Stock Controller ──
  ...
  Widget _buildFooter(VerifikasiOperatorHeader h) {
    return Padding(
            ...
            child: Builder(
              builder: (context) {
                ...
                return FilledButton.icon(
                  onPressed: (canVerify && !_isActing && !h.verifiedOperator)
                      ? _handleVerify
                      : null,
                  ...
                  label: Text(
                    h.verifiedOperator
                        ? 'Sudah Diverifikasi Production Controller'
                        : 'Verifikasi Production Controller',
                  ),
                );
              },
