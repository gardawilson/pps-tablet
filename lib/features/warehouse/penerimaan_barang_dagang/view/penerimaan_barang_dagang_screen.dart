// lib/features/warehouse/penerimaan_barang_dagang/view/penerimaan_barang_dagang_screen.dart
...
/// Layar Penerimaan Barang Dagang — konsep & desain sama persis dengan
/// Penerimaan Bahan Pendukung (Atlas table gaya broker, dialog untuk
/// "Penerimaan Baru" & "Detail", print label per barang dengan badge
/// jumlah cetak). Data masih dummy lokal (belum ada endpoint backend untuk
/// barang dagang). Print label memanfaatkan PDF viewer yang sama dengan
/// broker (`PdfPrintService` + `PdfViewerScreen` + Bluetooth print), PDF-nya
/// digenerate lokal karena belum ada endpoint server.
...
const _products = <_Product>[
  _Product('Furniture', 'SOFA SET MINIMALIS 3 DUDUKAN', 'P190 X L85 X T80 CM'),
  _Product('Furniture', 'KURSI CAFE ROTAN SINTETIS', 'P55 X L55 X T85 CM'),
  _Product('Furniture', 'MEJA MAKAN MARMER 6 KURSI', 'P180 X L90 X T75 CM'),
  _Product('Furniture', 'LEMARI PAKAIAN 3 PINTU', 'P150 X L55 X T200 CM'),
  _Product('Furniture', 'RAK SEPATU MULTIFUNGSI', 'P80 X L30 X T90 CM'),
  _Product('Furniture', 'TEMPAT TIDUR MINIMALIS 160', 'P160 X L200 CM'),
  _Product('Dekorasi', 'CERMIN HIAS DINDING BULAT', 'D60 CM'),
  _Product('Dekorasi', 'KARPET BULU RUANG TAMU', 'P200 X L150 CM'),
  _Product('Dekorasi', 'LAMPU HIAS GANTUNG RATTAN', 'D40 X T35 CM'),
  _Product('Dekorasi', 'VAS BUNGA KERAMIK MOTIF', 'T35 CM'),
  _Product('Aksesoris', 'SET MEJA KURSI TAMAN LIPAT', 'P120 X L70 X T72 CM'),
  _Product('Aksesoris', 'BANTAL SOFA DEKORASI 45X45', '45 X 45 CM'),
  _Product('Aksesoris', 'GORDEN BLACKOUT MOTIF POLOS', 'P270 X L140 CM'),
  _Product('Aksesoris', 'TALENAN KAYU JATI SET 3', 'P35 X L20 CM'),
];

const _suppliers = <String>[
  'PT. Mitra Furnindo Sejahtera',
  'CV. Dekorasi Nusantara',
  'PT. Graha Interior Indonesia',
];
...
Future<Uint8List> _buildLabelPdf(
  ...
  for (final item in items) {
    // Dummy payload — belum ada endpoint backend untuk barcode/label barang
    // dagang, jadi QR hanya mengenkode data yang sudah ada di lokal.
    final qrData = 'PBD|$no|${item.product.nama}|${item.qty}|$dateStr';
...
class _PenerimaanBarangDagangScreenState
  ...
  late final List<_ReceiptHistory> _history = [
    _ReceiptHistory(
      ...
      supplier: 'PT. Mitra Furnindo Sejahtera',
      note: 'Penerimaan rutin',
      items: [
        _ReceiptItem(_products[0], 6),
        _ReceiptItem(_products[2], 4),
        _ReceiptItem(_products[5], 8),
        _ReceiptItem(_products[6], 12),
      ],
    ),
    _ReceiptHistory(
      no: 'PBD-2026-08-0008',
      date: DateTime(2026, 8, 14),
      supplier: 'CV. Dekorasi Nusantara',
      note: 'Pengiriman barang dagang',
      items: [
        _ReceiptItem(_products[7], 10),
        _ReceiptItem(_products[8], 15),
        _ReceiptItem(_products[9], 20),
      ],
    ),
    _ReceiptHistory(
      no: 'PBD-2026-08-0007',
      date: DateTime(2026, 8, 12),
      supplier: 'PT. Graha Interior Indonesia',
      note: 'Penerimaan sesuai PO',
      items: [_ReceiptItem(_products[3], 5), _ReceiptItem(_products[4], 18)],
    ),
  ];
  ...
  String get _nextNo =>
      'PBD-2026-08-${(_history.length + 10).toString().padLeft(4, '0')}';
  ...
  Widget _buildItemsTable() {
    if (_newItems.isEmpty) {
      return const _EmptyBox(
        message:
            'Belum ada barang. Klik "Tambah Barang" untuk memilih barang dagang.',
      );
    }
    return Table(
      columnWidths: const {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Expanded(
              child: SingleChildScrollView(child: _buildBody(receipt)),
            ),
            const SizedBox(height: 16),
            _buildActions(),
          ],
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: printed ? _atlasBlue.withValues(alpha: 0.10) : Colors.grey.shade100,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
            color: printed ? _atlasBlue.withValues(alpha: 0.30) : Colors.grey.shade300,
          ),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
